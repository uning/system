...
sl_getconf() {
    ...
    if [ $# -eq 0 ]; then
        ...
        loginfo "Usage:sl_getconf confvarName confItemName";  
    fi 
...
sl_delconf () {
    ...
    if [ $# -eq 0 ]; then
        ...
        loginfo "sl_delconf error! Format:sl_delconf confvarName confItemName";
    fi 
...
sl_closeconf() {
    ...
    if [ $# -eq 0 ]; then
        ...
        do
            if [ x"${g_confVar[$i]}" = x"$1" ] ;then
		loginfo "close conf:${g_confName[$i]} succ!"
                g_confName[$i]="";
                g_confBuf[$i]="";
                g_confVar[$i]="";
                g_confSep[$i]="";
