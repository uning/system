#!/bin/bash
#===============================================================================
#          FILE:  init_mysql_data.sh
# 
#         USAGE:  ./init_mysql_data.sh 
# 
#   DESCRIPTION:  init a mysql runenv ,parameter port 
# 
#       OPTIONS:  ---
#  REQUIREMENTS:  ---
#          BUGS:  ---
...
#       VERSION:  1.0
#       CREATED:  2009年10月06日 09时51分55秒 CST
#      REVISION:  ---
#===============================================================================
...
file_tpl_my_conf=`dirname $0`/template-my.cnf
...
mlog_dir=$LOG/$name.$port
mkdir -p $mrun_dir
mkdir -p $mlog_dir
mkdir -p $mdata_dir/binlog

cat >$mrun_dir/my.cnf <<EOT
...
slow_query_log_file = $mrun_dir/slow.sql
basedir = $base_path
datadir = $mdata_dir
...
# system.
#show_slow_log_file=$mrun_dir/slow.sql
...
#sed_replace $file_tpl_my_conf '$port' $port >/tmp/my.cnf
#sed_replace /tmp/my.cnf '$mdata_dir' $mdata_dir >$mdata_dir/my.cnf
#exit


#control script
cat > $mrun_dir/mysqld_ctrl <<EOT
#!/bin/sh
...
function_start_mysql()
{
	...
	$base_path/bin/mysqld_safe --defaults-file=$mrun_dir/my.cnf 2>&1 > /dev/null &
}

function_stop_mysql()
{
    printf "Stoping MySQL...\n"
    $base_path /bin/mysqladmin -u ${mysql_user} -p${mysql_password} -S /tmp/mysql.sock.$port shutdown
}
...
if [ "\$1" = "start" ]; then
	...
	printf "Usage: $mrun_dir/mysqld_ctrl {start|stop|restart|kill}\n"
fi

EOT
chmod +x $mrun_dir/mysqld_ctrl

cat >$mrun_dir/mysql <<EOT
$base_path/bin/mysql -u$mysql_user -p$mysql_password
EOT
chmod +x $mrun_dir/mysql

#chown -R mysql:mysql $DATA/mysql
$base_path/bin/mysql_install_db --basedir=$base_path \
--datadir=$mdata_dir --user=hotel \
--defaults-file=$mrun_dir/my.cnf 

cat <<EOT
$base_path/bin/mysql -uroot -P$port 
